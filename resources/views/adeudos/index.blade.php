            ...
            <tbody>
                ...
                @forelse($adeudos as $index => $adeudo)
                    <tr class="{{ $adeudo->status == 'cancelado' ? 'text-muted bg-light' : '' }}">
                        ...
                        <td>
                            @if($adeudo->pagoDetalle)
                                <span
                                    class="text-success font-weight-bold">${{ number_format($adeudo->pagoDetalle->monto_pagado, 2) }}</span>
                            @else
                        ...
                        <td>{{ $adeudo->fecha_pago ? \Carbon\Carbon::parse($adeudo->fecha_pago)->format('d/m/Y') : '---' }}
                        </td>
                        <td class="text-center">
                            @if($adeudo->status != 'pagado' && $adeudo->status != 'cancelado')
                                <form action="{{ route('adeudos.cancelar', $adeudo->id) }}" method="POST" class="d-inline"
                            ...
                            @elseif($adeudo->status == 'pagado')
                                @php $pagoId = optional($adeudo->pagoDetalle)->pago_id; @endphp
                                @if($pagoId)
                                    <a href="{{ route('pagos.ticket', $pagoId) }}" target="_blank" class="btn btn-xs btn-info"
